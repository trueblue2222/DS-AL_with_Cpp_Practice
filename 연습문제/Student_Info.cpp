...
public:
    ...
    const T& operator[] (int index) const { return data[index]; } // operator overloading 오타 있었음. 문법상 operator[] 공백 없어야 함.
    T& at(int index) {
    ...
    T* end() { return data + n; }
    const T* end() const { return data + n; } // 반환값 보호 const는 썼는데 객체 보호 const를 안 썼음
